...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        embed_out = self.embedding(captions) 
        
        W = embed_out.shape[2]
        ## add positional encoding
        pos_encoder = PositionalEncoding(W)
        text_embed = pos_encoder(embed_out)
        D = features.shape[1]
        # project the features to the same space of the embedded captions:
        image_embed = self.visual_projection(features).unsqueeze(1)
        
        # preparing the mask:
        #tgt_mask: Array of shape (T, T) where tgt_mask[i,j] == 0 indicates token
        #  i in the source should not influence token j in the target.
        tgt_mask = torch.full((T, T), 1)
        tgt_mask = tgt_mask.T.tril(0).T

        # run the decoder:
        transformer_out = self.transformer(text_embed, image_embed, tgt_mask)
